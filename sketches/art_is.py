...
def extract_terms(tweet_text):
    ...
    cleaned_text = ' '.join([word for word in tweet_text.lower().split() \
                                 if not ignore_word(word)])
    words = re.findall(r'\w+', cleaned_text)
    return [word for word in words if (not word in ENGLISH_STOPWORDS)]

def report_term_counts(counts):
    """Report on the counts for ech term"""
    print str(counts)

# Keep a log of the tweets for debugging
def log_tweet(tweet):
    log = open(TWEET_LOGFILE, 'w+')
    print >>log, "TWEET: %s" % tweet
    log.close()
